...
class MasterControlApp:
    ...
    def init_logging_vars(self):
        ...
        self.filter_type = tk.StringVar(value="Python Filter")
        ...
        if os.path.exists(self.settings_file):
            try:
                with open(self.settings_file, 'r') as f:
                    ...
                    self.filter_type.set(data.get("filter_type", "Python Filter"))
                    self.read_type.set(data.get("read_type", "Tag Position"))
                    self.anchor_count.set(data.get("anchor_count", "4 Anchors (1 Listener)"))
                    ...
                    self.rep_save_pdf.set(data.get("rep_save_pdf", True))
                    self.rep_save_pngs.set(data.get("rep_save_pngs", True))
            ...
            try:
                with open(self.uwb_config_file, 'r') as f:
                    ...
                    for i, mod in enumerate(loaded_modules):
                        if i < len(self.uwb_modules):
                            self.uwb_modules[i].update(mod)
            except Exception as e:
    ...
    def save_settings(self):
        ...
        log_data = {
            ...
            "anchor_count": self.anchor_count.get(),
            "rep_name": self.rep_name.get(),
            "rep_notes": self.rep_notes.get(),
            "rep_show_anchors": self.rep_show_anchors.get(),
            "rep_save_pdf": self.rep_save_pdf.get(),
            "rep_save_pngs": self.rep_save_pngs.get()
        }
        try:
    ...
    def setup_logging_tab(self):
        ...
        tk.Label(hardware_frame, text="UWB Port 1:").grid(row=0, column=1, padx=5, sticky="e")
        tk.Entry(hardware_frame, textvariable=self.uwb_port1, width=15).grid(row=0, column=2)

        tk.Checkbutton(hardware_frame, text="Enable UWB Port 2", variable=self.enable_uwb_port2).grid(row=1, column=1,
                                                                                                      sticky="w")
        tk.Entry(hardware_frame, textvariable=self.uwb_port2, width=15).grid(row=1, column=2)

        tk.Frame(hardware_frame, height=10).grid(row=2, column=0, columnspan=3)

        tk.Checkbutton(hardware_frame, text="Enable Ground Truth", variable=self.enable_gt).grid(row=3, column=0,
                                                                                                 sticky="w")
        gt_dropdown = ttk.Combobox(hardware_frame, textvariable=self.gt_type, values=["OptiTrack", "GPS"],
        ...
        tk.Checkbutton(routing_frame, text="Send Data to MATLAB", variable=self.send_matlab).grid(row=0, column=0,
                                                                                                  sticky="w", padx=5,
                                                                                                  pady=2)
        tk.Checkbutton(routing_frame, text="Send Data to ROS", variable=self.send_ros).grid(row=0, column=1, sticky="w",
                                                                                            padx=5, pady=2)

        tk.Label(routing_frame, text="Filter Target:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        ttk.Combobox(routing_frame, textvariable=self.filter_type, values=["Python Filter", "MATLAB Filter"],
        ...
        ttk.Combobox(routing_frame, textvariable=self.anchor_count,
                     values=["4 Anchors (1 Listener)", "8 Anchors (2 Listeners)"], state="readonly", width=22).grid(
    ...
    def run_master_process(self):
        ...
        self.current_session_dir = os.path.join(base_dir, session_name)

        if not os.path.exists(self.current_session_dir):
            os.makedirs(self.current_session_dir)

        # Restore configuration text file logging
        ...
        with open(config_file, 'w') as f:
            ...
            if self.enable_gt.get():
                ...
                f.write("GT_CONFIG: Disabled\n")

            f.write(
                f"ROUTING_CONFIG: send_matlab={self.send_matlab.get()}, filter={self.filter_type.get()}, read={self.read_type.get()}, scale={self.anchor_count.get()}, send_ros={self.send_ros.get()}\n")

        threads = []

        # Package the UWB config including the new routing variables
        uwb_config = {
        ...
        if self.enable_uwb.get():
            # Notice self.data_queue is the 4th item in the args tuple here as well
    ...
    def setup_report_tab(self):
        ...
        tk.Label(report_frame, text="Generate Standalone Report", font=("Arial", 14)).grid(row=0, column=0,
                                                                                           columnspan=3, pady=(0, 20),
                                                                                           sticky="w")

        tk.Label(report_frame, text="Measurement Name:").grid(row=1, column=0, sticky="w", pady=5)
        tk.Entry(report_frame, textvariable=self.rep_name, width=40).grid(row=1, column=1, columnspan=2, pady=5,
                                                                          sticky="w")

        tk.Label(report_frame, text="Notes:").grid(row=2, column=0, sticky="w", pady=5)
        tk.Entry(report_frame, textvariable=self.rep_notes, width=40).grid(row=2, column=1, columnspan=2, pady=5,
                                                                           sticky="w")

        tk.Checkbutton(report_frame, text="Show Anchors", variable=self.rep_show_anchors).grid(row=3, column=0,
                                                                                               sticky="w", pady=10)
        tk.Checkbutton(report_frame, text="Save PDF Report", variable=self.rep_save_pdf).grid(row=3, column=1,
                                                                                              sticky="w", pady=10)
        tk.Checkbutton(report_frame, text="Save Individual PNGs", variable=self.rep_save_pngs).grid(row=4, column=0,
                                                                                                    sticky="w")

        btn_frame = tk.Frame(report_frame, pady=20)
        btn_frame.grid(row=5, column=0, columnspan=3, sticky="w")

        tk.Button(btn_frame, text="Save Report Settings", command=self.save_settings, width=20).pack(side=tk.LEFT,
                                                                                                     padx=(0, 10))
    ...
    def manual_report_trigger(self):
        ...
        if folder:
            report_config = {
                ...
                'save_pdf': self.rep_save_pdf.get(),
                'save_pngs': self.rep_save_pngs.get()
            }
    ...
    def setup_anchor_tab(self):
        ...
        tk.Button(file_btn_frame, text="Load Config", command=self.load_uwb_config).pack(side=tk.LEFT, fill=tk.X,
                                                                                         expand=True, padx=(0, 2))
        tk.Button(file_btn_frame, text="Save As...", command=self.save_uwb_config_as).pack(side=tk.LEFT, fill=tk.X,
        ...
        tk.Button(list_btn_frame, text="+ Add Module", command=self.add_module).pack(side=tk.LEFT, fill=tk.X,
                                                                                     expand=True, padx=(0, 2))
        tk.Button(list_btn_frame, text="- Remove", command=self.remove_module).pack(side=tk.LEFT, fill=tk.X,
                                                                                    expand=True, padx=(2, 0))
        ...
        tk.Label(detail_frame, text="Module Settings", font=("Arial", 12, "bold")).grid(row=0, column=0, columnspan=2,
                                                                                        sticky="w", pady=(0, 15))
        ...
        type_dropdown = ttk.Combobox(detail_frame, textvariable=self.mod_type, values=["Anchor", "Tag", "Listener"],
        ...
        tk.Checkbutton(toggles_frame, text="LEDs Enabled", variable=self.mod_led_enabled).pack(side=tk.LEFT,
                                                                                               padx=(10, 0))
        ...
        tk.Button(action_frame, text="Apply Edit to List", command=self.apply_module_edit, width=15).pack(side=tk.LEFT,
                                                                                                          padx=(0, 10))
        tk.Button(action_frame, text="Push Configuration via BLE", command=self.push_ble_config, bg="purple",
                  fg="white", width=25).pack(side=tk.LEFT)
    ...
    def update_live_plot(self):
        ...
        try:
            while True:
                # Capture the Z coordinate here
                source, x, y, z = self.data_queue.get_nowait()
                if source == 'UWB':
        ...
        if updated:
            # Use set_data_3d instead of set_data
